...
const EMPTY_BEAN_STATS = { normal: 0, golden: 0, speed: 0, slow: 0, ghost: 0 };
...
const SnakeGame = ({ user }) => {
  ...
  // 本局统计 refs
  const startTimeRef = useRef(0);          // 本局开始时间戳
  const beanStatsRef = useRef({ ...EMPTY_BEAN_STATS }); // 各类豆统计
  const submittedRef = useRef(false);      // 防止同一局重复提交
  ...
  const submitScore = useCallback(async () => {
    if (!user) {
      ...
      setSubmitMessage('未登录,本局成绩未上传。登录后可上榜排行榜。');
      return;
    }

    const duration_ms = Date.now() - startTimeRef.current;
    const breakdown = { ...beanStatsRef.current };
    const beans_eaten = Object.values(breakdown).reduce((a, b) => a + b, 0);
    const snake_length = snake.length;

    setSubmitStatus('submitting');
    setSubmitMessage('正在上传成绩...');
    ...
    try {
      ...
      setSubmitMessage(`成绩已上榜: ${score} 分 / ${(duration_ms / 1000).toFixed(1)}s`);
    } catch (err) {
      setSubmitStatus('error');
      setSubmitMessage(`上传失败: ${err.message || err}`);
    }
  }, [user, score, snake.length]);

  // game over 触发提交(用 ref 防重复)
  useEffect(() => {
    if (gameOver && !submittedRef.current && gameStarted) {
  ...
  const moveSnake = useCallback(() => {
    setSnake(currentSnake => {
      ...
      if (eatenFood) {
        ...
        // 统计各类豆
        if (beanStatsRef.current[eatenFood.type] !== undefined) {
          beanStatsRef.current[eatenFood.type] += 1;
        }
  ...
  const startGame = () => {
    ...
    beanStatsRef.current = { ...EMPTY_BEAN_STATS };
        ...
        {!gameStarted ? (
          ...
          <div className="game-over">
            ...
            <p className="game-duration">
              时长: {startTimeRef.current ? ((Date.now() - startTimeRef.current) / 1000).toFixed(1) : 0}s
              {' · '}吃豆: {Object.values(beanStatsRef.current).reduce((a, b) => a + b, 0)}
            </p>

            {/* 分数提交状态 */}
            {submitStatus !== 'idle' && (
              <div className={`submit-status submit-${submitStatus}`}>
          ...
          <div className="game-info">
            ...
            {!user && (
              <p className="login-hint">💡 登录后,游戏结束时成绩会自动上传排行榜</p>
            )}
          </div>
        )}
      </div>
